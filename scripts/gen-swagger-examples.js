...
if (registerContent) {
  ...
  console.log(" Example для POST /register добавлен");
}
...
if (offerContent) {
  ...
  console.log(" Example для POST /offer добавлен");
}

const reviewsPostContent = doc?.paths?.["/comments/{offerId}"]?.post?.requestBody?.content?.["application/json"];
if (reviewsPostContent) {
  reviewsPostContent.example = genReviewExample();
  console.log(" Example для POST /comments/{offerId} добавлен");
}

fs.writeFileSync(SWAGGER_PATH, YAML.stringify(doc), "utf-8");

console.log(" Готово! Примеры сгенерированы в", SWAGGER_PATH);